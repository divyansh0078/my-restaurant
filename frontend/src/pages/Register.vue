...
export default {
    ...
    if (this.registerObj.phone.length !== 10) {
        this.errorObj.phoneErr.push('Phone number must have exactly 10 digits');
    }

    if (!/^\d{10}$/.test(this.registerObj.phone)) {
        this.errorObj.phoneErr.push('Phone number can only contain digits');
    }

    // Optional: Uncomment if phone should start with a specific prefix
    // if (!this.registerObj.phone.startsWith('84')) {
    //     this.errorObj.phoneErr.push('Phone number must start with 84');
    // }
}
    ...
    const dateInput = new Date(this.registerObj.birth);
    ...
    if (isNaN(dateInput.getTime())) {
        ...
        date150YearsAgo.setFullYear(today.getFullYear() - 150);

        if (dateInput < date150YearsAgo || dateInput > today) {
            this.errorObj.birthErr.push("Birthday must be between the past 150 years and today");
        }
    }
}
